...
```mermaid
graph TD
    A[Start Bot] --> B[Placing Initial BUY Order]
    B --> C{Market Movement}
    C -- Price Rises --> D[Profits Grow]
    D --> E{Combined Profit >= TP USD?}
    E -- Yes --> F[Close All Positions & Lock in Profit]
    E -- No --> C
    
    C -- Price Drops by 'Pip Threshold' --> G[Place SELL Hedge Order]
    G --> H{Price Direction}
    H -- Continues Dropping --> I[Hedge is Active: Loss is Locked/Frozen]
    I --> J{Combined Balance hits SL or TP?}
    J -- Yes --> K[Close All Positions]
    J -- No --> I
    
    H -- Price Rises above latest SELL + threshold --> L[Close old SELL & Place new higher SELL]
    L --> C
```
